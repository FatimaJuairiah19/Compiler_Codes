...
void findErrors()
{
    ...
    while((ch=fgetc(p1))!=EOF)
    {
        if(ch>='a' && ch<='z')
        {
            ...
            else if(j==2)
            {
                ...
                if(strcmp(ta,"for")==0)
                {
                    checkFor=1;     // additional checking in for loop

                }
                else
                {
                    checkFor=0;   // additional checking in for loop
                }
    ...
    {
       //cout<<endl<<"Duplicate token ; at line "<<semiLine[i].first;
